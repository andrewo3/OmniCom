...
int main(int argc, char ** argv) {
    ...
    command_stream << "g++-11 -DROM_NAME=\"\\\"" << fixed(argv[1]);
    command_stream << "\\\"\" -DDATAROM="<<xxdstring(argv[1])<<" -DDATALENGTH="<<xxdstring(argv[1])<<"_len ";
    get_filename(&argv[1]);
    command_stream << "-g -F./bin -L./lib -Iinclude/unix -Isrc/ntsc-filter -Iinclude/universal src/ntsc-filter/crt_core.c src/ntsc-filter/crt_ntsc.c src/util.cpp src/rom.cpp src/cpu.cpp src/cpu_helper.cpp src/ppu.cpp src/apu.cpp src/mapper.cpp src/main.cpp -framework SDL2 -framework OpenGL -lGLEW -o \"bin/" << argv[1] << "\"";
