...
int usage_error() {
    printf("Usage: omnicom rom_filename\n");
    return -1;
}
...
int main(int argc, char ** argv) {
    ...
    command_stream << "g++ -O3 -DGLEW_STATIC -DROM_NAME=\"\\\"" << fixed(argv[1]);
    command_stream << "\\\"\" -DDATAROM="<<xxdstring(argv[1])<<" -DDATALENGTH="<<xxdstring(argv[1])<<"_len ";
    get_filename(&argv[1]);
    command_stream << "-static -Iinclude\\win32 -Isrc\\ntsc-filter -Iinclude\\universal -Isrc\\imgui -Isrc\\imgui\\backends src/imgui/backends/*.cpp src/imgui/*.cpp src/*.cpp src/NES/*.cpp src/SNES/*.cpp src/window/*.cpp src/ntsc-filter/crt_core.c src/ntsc-filter/crt_ntsc.c -Llib -static-libgcc -static-libstdc++ -Wl,-Bstatic -lmingw32 -lSDL2main -lSDL2 -lm -lkernel32 -luser32 -lgdi32 -lwinmm -limm32 -lole32 -loleaut32 -lversion -luuid -ladvapi32 -lsetupapi -lshell32 -ldinput8 -lglew32s -lopengl32 -o \"" << original_path.string() << "\\" << argv[1] << ".exe\" -mwindows";
