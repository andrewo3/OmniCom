...
int main(int argc, char ** argv) {
    ...
    uint32_t MAX_PATH = 256;
    //GetModuleFileName(NULL,path,256);
    std::filesystem::path cpath = std::filesystem::canonical("/proc/self/exe");;
    std::cout << "EXEC: " << cpath << "\n";
    ...
    command_stream << "em++ -std=c++17 -O3 -sUSE_PTHREADS=1 -sEXPORTED_FUNCTIONS=\"_main\",\"_changeRom\",\"_saveCurrentRom\" -sFORCE_FILESYSTEM=1 -sPTHREAD_POOL_SIZE=4 -sASSERTIONS -sSTACK_SIZE=1048576 -sWASM_BIGINT=1 -DWEB=1 ";
    ...
    command_stream << "-Isrc/ntsc-filter -Iinclude/universal -Isrc/imgui -Isrc/imgui/backends src/imgui/backends/*.cpp src/imgui/*.cpp src/ntsc-filter/crt_core.c src/ntsc-filter/crt_ntsc.c src/*.cpp src/SNES/*.cpp src/NES/*.cpp src/window/*.cpp -lidbfs.js -Llib -sUSE_SDL=2 -o web/static/index.js";
